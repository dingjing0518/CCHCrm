...
public class AdminMemberController extends BaseController {
	...
	private final String QUICK_CREATE = "admin/member/quick_create";
	...
	public List<Card> findValidCard() {
		return cardService.findByValid(ValidEnum.VALID);
	}
	...
	//添加会员快捷入口
	@RequestMapping(value = "/quickCreate", params = "form", method = RequestMethod.GET)
	public String cardForm( Model uiModel) {
		Member member = new Member();
		member.setMemberDetail(new MemberDetail());
		uiModel.addAttribute("member", member);
		return QUICK_CREATE;
	...
	public String cardForm( @Valid @ModelAttribute("member") Member member,
			BindingResult result, Model uiModel, RedirectAttributes redirectAttributes) {
		if (result.hasErrors()) {
			uiModel.addAttribute("message", failMessage(messageSource));
		}
	...
	public Status scoreAdd(@RequestBody UpdateScoreJson json) {
		...
		Member member = memberService.addScore(new Member(json.getId()), json.getScore(), ADMIN_MANUAL_ADD_SCORE,
				ScoreHistoryEnum.ADMIN, json.getRemark(),"",userContext.getCurrentUser().getName());
	...
	//停车记录
	@RequestMapping(value = "/{id}/parkingPayHistory", method = RequestMethod.GET)
	@ResponseBody
	public Status parkingHistory(@PathVariable("id") Long id,
						  @PageableDefault(page = INIT_PAGE, size = PAGE_SIZE, sort = "createdTime", direction = Sort.Direction.DESC) Pageable pageable) {
		return new Success<>(
				parkingPayHistoryService.toJson(parkingPayHistoryService.findByMember(new Member(id), pageable), pageable));
	}
	//停车券兑换记录
	@RequestMapping(value = "/{id}/parkingCouponDetail", method = RequestMethod.GET)
	@ResponseBody
	public Status exchangeHistory(@PathVariable("id") Long id,
								 @PageableDefault(page = INIT_PAGE, size = PAGE_SIZE,sort ="exchangeTime",direction =Sort.Direction.DESC) Pageable pageable) {
		return new Success<>(parkingCouponDetailService.toJson(parkingCouponDetailService.findByMember(new Member(id), pageable),pageable));
	}
