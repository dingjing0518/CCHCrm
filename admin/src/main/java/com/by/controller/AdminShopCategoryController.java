...
public class AdminShopCategoryController extends BaseController {
	...
	public String subDetail(Model uiModel,@PathVariable("parentId") int parentId) {
		uiModel.addAttribute("category", new ShopCategory());
		uiModel.addAttribute("parantId", parentId);
		return FORM;
	}
	...
	public String add(Model uiModel, @PathVariable("parentId") int id,
		...
		return REDIRECT + sc.getParent().getId();
	}
	...
	public String update(Model uiModel, @PathVariable("id") int id,
		...
		ShopCategory sc = service.update(category);
		return REDIRECT + sc.getParent().getId() + "/child/" + category.getId();
	}
