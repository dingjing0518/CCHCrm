...
public class ShopCouponMemberServiceImpl implements ShopCouponMemberService {
	...
	@Override
	public void exchangeCoupon(Member member, ShopCoupon coupon) {
		List<ShopCouponMember> lists = findByCouponAndMember(coupon, member);
		if (coupon.isEffective(Calendar.getInstance())) {
			// 如果不能重复
			if (!coupon.isDuplicateCoupon()) {
				if (lists.size() > 0) {
					throw new AlreadyExchangeException();
				}
			}
			memberService.minusScore(member, coupon.getScore() , reason, ScoreHistoryEnum.COUPONEXCHANGE,"","", "System");
			save(coupon, member);
		} else {
	...
	public void exchangeCoupon(Member member, ShopCoupon coupon, int total) {
		...
		if (shopCoupon.isValid() && shopCoupon.isEffective(Calendar.getInstance())) {
			...
			memberService.minusScore(m, shopCoupon.getScore() * count, reason, ScoreHistoryEnum.COUPONEXCHANGE,"", "" , "System");
