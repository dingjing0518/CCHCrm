...
public class GiftCouponMemberServiceImpl implements GiftCouponMemberService {
	...
	public void exchangeCoupon(GiftCoupon coupon, Member member, int total) {
		...
		if (sourceCoupon.isValid() && sourceCoupon.isEffective(Calendar.getInstance())) {
			...
			memberService.minusScore(m, sourceCoupon.getScore() * count, reason, ScoreHistoryEnum.COUPONEXCHANGE,"", "", "System");
	...
	@Override
	public GiftCouponMember findByCodeAndValid(String code) {
		return repository.findByCodeAndValid(code, ValidEnum.VALID, Calendar.getInstance());
	}
	
	@Override
	public GiftCouponMember findByCode(String code) {
