...
public class MemberServiceImpl implements MemberService {
	...
	@Override
	public Member addScore(Member member, int total, String reason, ScoreHistoryEnum type, String remark, String tradeNo, String operator) {
		Member source = repository.findOne(member.getId());
		source.setScore(source.getScore() + total);
		scoreAddHistoryService.save(member, total, reason);
		scoreHistoryService.save(member, total, reason, type, remark, tradeNo, operator);
		source.setSumScore(source.getSumScore() + total);
		return source;
	}

	public Member minusScoreBase(Member member, int total, String reason, ScoreHistoryEnum type, String remark, String tradeNo, String operator,
		...
		if (source.getScore() >= total) {
			...
			scoreHistoryService.save(member, -member.getScore(), reason, type,remark, tradeNo, operator);
		}
		source.setScore(source.getScore() - total);
		return source;
	}
	...
	@Override
	public Member minusScore(Member member, int total, String reason, ScoreHistoryEnum type, String remark, String tradeNo, String operator) {
		return minusScoreBase(member, total, reason, type, remark, tradeNo,operator, n -> {
			throw new NotEnoughScoreException();
		});
	}

	@Override
	public Member minusScoreAll(Member member, int total, String reason, ScoreHistoryEnum type, String remark, String tradeNo, String operator) {
		return minusScoreBase(member, total, reason, type, remark, tradeNo, operator, n -> {
			scoreAddHistoryService.deleteByMember(n);
			scoreHistoryService.save(member, n.getScore(), reason, type, remark,tradeNo, operator);
			n.setScore(0);
		});
	}
