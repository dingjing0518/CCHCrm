...
public class CouponServiceImpl implements CouponService {
	...
	@Override
	@Transactional(readOnly = true)
	public Page<CouponJson> findAllByType(String type, Pageable pageable) {
		Calendar today = Calendar.getInstance();
		
		Page<Coupon> coupons = null;
		Long total = 0L;
		List<CouponJson> couponJson = null;
		
		switch (type) {
			case "p":
				coupons = repository.findAllByValidAndDateBetweenAndType(ValidEnum.VALID, today, pageable,
						ParkingCoupon.class);
				couponJson = coupons.getContent().stream().map(i -> i.toParkCouponJson())
						.collect(Collectors.toList());
			...
			case "g":
				coupons = repository.findAllByValidAndDateBetweenAndType(ValidEnum.VALID, today, pageable,
						GiftCoupon.class);
				couponJson = coupons.getContent().stream().map(i -> i.toGiftCouponJson())
						.collect(Collectors.toList());
				total = coupons.getTotalElements();
				return new PageImpl<>(couponJson, pageable, total);
			case "s":
				coupons = repository.findAllByValidAndDateBetweenAndType(ValidEnum.VALID, today, pageable,
						ShopCoupon.class);
				couponJson = coupons.getContent().stream().map(i -> i.toCouponJson())
						.collect(Collectors.toList());
				total = coupons.getTotalElements();
				return new PageImpl<>(couponJson, pageable, total);
			default:
	...
	@Override
	@Transactional(readOnly = true)
	public Page<CouponJson> findByMemberAndType(Member member,String type, Pageable pageable) {
		Calendar today = Calendar.getInstance();
		Long total = 0L;
		switch (type) {
			case "p":
				Page<ParkingCouponMember> parkingCouponlists = parkingCouponMemberService.findByMemberPageable(member,
						pageable);
				List<CouponJson> parkingjson = parkingCouponlists.getContent().stream().map(i -> i.toCouponJson())
						.collect(Collectors.toList());
				total = parkingCouponlists.getTotalElements();
				return new PageImpl<>(parkingjson, pageable, total);
			case "g":
				Page<GiftCouponMember> giftCouponList = giftCouponMemberRepository.findByMemberAndValid(member,
						ValidEnum.VALID, today, pageable);
				List<CouponJson> giftJson = giftCouponList.getContent().stream().map(i -> i.toCouponJson())
						.collect(Collectors.toList());
				total = giftCouponList.getTotalElements();
				return new PageImpl<>(giftJson, pageable, total);
			case "s":
				Page<ShopCouponMember> shopCouponList = shopCouponMemberRepository.findByMemberAndValid(member,
						ValidEnum.VALID, today, pageable);
				List<CouponJson> shopJson = shopCouponList.getContent().stream().map(i -> i.toCouponJson())
						.collect(Collectors.toList());
				total = shopCouponList.getTotalElements();
				return new PageImpl<>(shopJson, pageable, total);
			default:
				List<CouponJson> list = new ArrayList<CouponJson>();
				return new PageImpl<>(list,pageable,0);
		}
