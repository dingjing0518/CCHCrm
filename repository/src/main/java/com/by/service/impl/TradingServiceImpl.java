...
public class TradingServiceImpl implements TradingService {
	private final String reason = "人工积分";
	...
	@Override
	public Trading manualAddScore(ManualForm form, String operator) {
...
//		Trading t = repository.findByCode(form.getFlowCode());
		Trading t = findByCodeAndShop(form.getFlowCode(),form.getShopName());
		//没有人工积分
		if (t == null) {
			...
			memberService.addScore(member, score, reason, ScoreHistoryEnum.TRADE, form.getRemark(), form.getFlowCode(), operator);
	...
	public Trading fromPoster(TradingRequestJson json) {
		...
		if (json.getMobile() != null) {
			...
			json.setScore(score);
			memberService.addScore(member, score, TRADING_SCORE, ScoreHistoryEnum.TRADE,"","","System");
		}
		return save(json);
	}
