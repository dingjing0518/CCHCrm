...
public interface ShopCouponMemberRepository extends PagingAndSortingRepository<ShopCouponMember, Long> {
	...
	List<ShopCouponMember> findByMemberAndValid(@Param("member") Member member, @Param("valid") ValidEnum valid,
			@Param("today") Calendar today);

	@Query("select sc from ShopCouponMember sc where sc.usedTime is null and sc.member=:member and sc.coupon.valid=:valid and ((:today<sc.coupon.couponEndTime) or (sc.coupon.couponEndTime is null)) order by sc.coupon.couponEndTime")
	Page<ShopCouponMember> findByMemberAndValid(@Param("member") Member member, @Param("valid") ValidEnum valid,
	...
	List<ShopCouponMember> findByMemberAndShop(@Param("name") String name, @Param("shopKey") String shopKey,
			@Param("valid") ValidEnum valid, @Param("today") Calendar today);
}
